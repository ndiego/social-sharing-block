 ...
 */
function render_block_outermost_social_link( $attributes, $content, $block ) {

	$service      = ( isset( $attributes['service'] ) ) ? $attributes['service'] : 'share';
	$url          = block_outermost_social_link_get_url( $service );
	$rendered_url = 'print' === $service ? $url : esc_url( $url );
	$label        = ( isset( $attributes['label'] ) ) ? $attributes['label'] : block_outermost_social_link_get_label( $service );
	...
	$icon               = block_outermost_social_link_get_icon( $service );
	$wrapper_attributes = get_block_wrapper_attributes(
		array(
			'class' => 'outermost-social-share-link outermost-social-share-link-' . $service . $class_name,
			'style' => block_outermost_social_link_get_color_styles( $block->context ),
		)
	);

	return '<li ' . $wrapper_attributes . '><a href="' . $url . '" aria-label="' . esc_attr( $label ) . '" title="' . esc_attr( $label ) . '" ' . $attribute . ' class="wp-block-outermost-social-share-link-anchor "> ' . $icon . '</a></li>';
}

/**
 * Registers the `core/social-link` blocks.
 */
function register_block_outermost_social_link() {
	register_block_type_from_metadata(
		__DIR__,
		array(
			'render_callback' => 'render_block_outermost_social_link',
		)
	);
}
add_action( 'init', 'register_block_outermost_social_link' );
 ...
 */
function block_outermost_social_link_get_icon( $service ) {
	$services = block_outermost_social_link_services();
 ...
 * @return array|string
 */
function block_outermost_social_link_services( $service = '', $field = '' ) {
 ...
 */
function block_outermost_social_link_get_color_styles( $context ) {
	$styles = array();
